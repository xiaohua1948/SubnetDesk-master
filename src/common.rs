...
/// Internal application token (branding).
///
/// Every mechanical name is derived from it: the exe file name, the Windows
/// service name, the registry root, the install folder, the URL protocol and
/// the temporary file extension. It must therefore stay a single token with no
/// spaces (`sc create`, `taskkill /IM` and the URL protocol all break on
/// spaces). The name shown to the user is [`APP_DISPLAY_NAME`].
pub const DEFAULT_APP_NAME: &str = "lenovo-display";
...
/// prefix, ...), i.e. as the very first statement of `core_main()`.
pub fn init_app_name() {
    *hbb_common::config::APP_NAME.write().unwrap() = DEFAULT_APP_NAME.to_owned();
}
...
pub fn is_custom_client() -> bool {
    !matches!(
        get_app_name().as_str(),
        "RustDesk" | "SubnetDesk" | DEFAULT_APP_NAME
    )
}
