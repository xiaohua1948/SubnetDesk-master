...
pub fn core_main() -> Option<Vec<String>> {
    // Branding. Must happen before anything reads the app name: paths, config
    // files, the Windows service name and the uri prefix are all derived from it.
    crate::common::init_app_name();
    ...
    // Set by `--show`, the argument the tray uses to open the main window.
    let mut show_main_window = false;
    ...
    for arg in std::env::args() {
        if i == 0 {
            ...
            if arg == "--elevate" {
                ...
                // Consumed on every platform, only Windows acts on it: it is
                // the argument the tray uses to raise the main window.
                #[cfg(target_os = "windows")]
                {
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        #[cfg(windows)]
        {
            ...
            // Launching the installed exe without arguments (desktop/start menu
            // shortcut, double click, auto start, end of the MSI install) only
            // keeps the tray alive: no main window and no taskbar entry. The
            // tray opens the main window with `--show`.
            // Portable runs keep the old behaviour, they need the in-process
            // server that the main window would otherwise host.
            if args.is_empty() && !show_main_window && crate::platform::is_installed() {
                if !crate::check_process("--tray", true) {
                    hbb_common::allow_err!(crate::run_me(vec!["--tray"]));
                }
                return None;
