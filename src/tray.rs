...
fn make_tray(show_icon: bool) -> hbb_common::ResultType<()> {
    ...
    let open_func = move || {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            // Use `run_me` instead.
            // `--show` is required because a plain launch (no args) only starts
            // the tray -- see `core_main()`. `allow_multiple_instances` in
            // `flutter/windows/runner/main.cpp` then raises the running window
            // instead of opening a second one.
            crate::run_me::<&str>(vec!["--show"]).ok();
        }
        #[cfg(target_os = "linux")]
        {
